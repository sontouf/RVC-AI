...
11. **단위 테스트** — focal·stub/driver·커버리지 (`develop-unit-tests-gtest`)
12. **통합 테스트** — 모듈 결합(GTest, 단위 다음 단계)

## 6. 시스템 테스트 (비-GTest, GUI·시뮬레이터)

13. `arch/vnv/system-tests.md` — **≥30** 케이스, pos/neg (`plan-system-tests-gui`). CI에서는 **통합 테스트 통과 후** 실행.

## 7. CI/CD

14. `.github/workflows/` — **빌드 → 단위 → 통합 → 시스템** 순 (`configure-cicd-github-actions`)

## 8. 정적 분석 (시스템 테스트 통과 후)

15. 전체 코드 정적 분석, **`reports/static-analysis/`** 에 결과 저장 (`archive-static-analysis-report`)
